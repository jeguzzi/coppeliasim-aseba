...
private:
  ...
  void battery(float time_step) {
    ...
    float intensity[3] = {1.0f, (bat > bat_middle) ? 1.0f : 0.0f,
                          (bat > bat_high) ? 1.0f : 0.0f};
    if (bat <= bat_low) {
      // 60 ms on + 60 ms off
  ...
  void sound_mic(float time_step) {
    // In the real firmware, it runs at 60 Hz in the loop that computes the
    // sound mean value, and possibly emit the event.
    ...
    if (t) {
      ...
      if (i > t && i > sound_led_intensity) {
        ...
        sound_led_intensity =
            std::max(0.0, sound_led_intensity - time_step * 60.0 / 255.0);
      }
    } else {
  ...
  void leds_buttons(float time_step) {
    ...
    if (button_intensity[2]) {
      for (int i = 0; i < 4; i++)
        robot.set_led_intensity(LED::BUTTON_UP + i,
                                button_intensity[Button::CENTER]);
    } else {
      robot.set_led_intensity(LED::BUTTON_DOWN,
                              button_intensity[Button::BACKWARD]);
      robot.set_led_intensity(LED::BUTTON_LEFT, button_intensity[Button::LEFT]);
      robot.set_led_intensity(LED::BUTTON_UP,
                              button_intensity[Button::FORWARD]);
      robot.set_led_intensity(LED::BUTTON_RIGHT,
                              button_intensity[Button::RIGHT]);
    }
  }
  ...
  void leds_prox(float time_step) {
    ...
    for (int i = 0; i < 7; i++) {
      float intensity =
          (robot.get_proximity_value(i) - robot.proximity_min_value) / h;
      if (i < 2) {
  ...
  void leds_rc5(float time_step) {
    robot.set_led_intensity(LED::RIGHT_RED,
                            robot.received_rc_message() ? 1.0 : 0.0);
  }
  ...
  void leds_ntc(float time_step) {
    ...
    if (temp_counter > temp_period) {
      ...
      temp_counter = 0;
      float r = std::clamp((temperature - temp_cold) / (temp_hot - temp_cold),
                           0.0f, 1.0f);
...
public:
  ...
  void set(uint8_t value) { behavior = value; }
...
enum { TOP_TEXTURE, BOTTOM_TEXTURE, LED_TEXTURE };
...
public:
  ...
  LEDTexture(std::vector<TextureRegion> regions, unsigned index,
...
std::array<LEDTexture, LED::COUNT> led_textures = {
    LEDTexture{{{130, 535, 350, 488},
               ...
               {LED::BUTTON_DOWN, LED::BUTTON_LEFT, LED::BUTTON_RIGHT,
                ...
                LED::IR_BACK_0, LED::IR_BACK_1, LED::LEFT_RED, LED::LEFT_BLUE,
                LED::RIGHT_BLUE, LED::RIGHT_RED, LED::BATTERY_0, LED::BATTERY_1,
                LED::BATTERY_2}},
    LEDTexture{{{563, 38, 117, 301}, {651, 731, 210, 113}},
               BOTTOM_TEXTURE,
               {LED::IR_FRONT_0, LED::IR_FRONT_1, LED::IR_FRONT_2,
                LED::IR_FRONT_3, LED::LEFT_RED, LED::LEFT_BLUE, LED::RIGHT_BLUE,
                LED::RIGHT_RED}},
    LEDTexture{{{565, 344, 224, 192}},
               BOTTOM_TEXTURE,
               {LED::IR_FRONT_2, LED::IR_FRONT_3, LED::IR_FRONT_4,
                LED::IR_FRONT_5, LED::LEFT_RED, LED::LEFT_BLUE, LED::RIGHT_BLUE,
                LED::RIGHT_RED}},
...
// str(', ').join(map(lambda x: str(int(x*255)),
// pow(np.arange(0., 1.001, 1./255.), 0.30)))
static const uint32_t pow_030_table[256] = {
    0,   48,  59,  67,  73,  78,  82,  86,  90,  93,  96,  99,  101, 104, 106,
    108, 111, 113, 115, 117, 118, 120, 122, 123, 125, 127, 128, 130, 131, 132,
    134, 135, 136, 138, 139, 140, 141, 142, 144, 145, 146, 147, 148, 149, 150,
    151, 152, 153, 154, 155, 156, 157, 158, 159, 160, 160, 161, 162, 163, 164,
    165, 166, 166, 167, 168, 169, 169, 170, 171, 172, 173, 173, 174, 175, 175,
    176, 177, 178, 178, 179, 180, 180, 181, 182, 182, 183, 184, 184, 185, 185,
    186, 187, 187, 188, 189, 189, 190, 190, 191, 191, 192, 193, 193, 194, 194,
    195, 195, 196, 197, 197, 198, 198, 199, 199, 200, 200, 201, 201, 202, 202,
    203, 203, 204, 204, 205, 205, 206, 206, 207, 207, 208, 208, 209, 209, 210,
    210, 211, 211, 212, 212, 213, 213, 213, 214, 214, 215, 215, 216, 216, 217,
    217, 217, 218, 218, 219, 219, 220, 220, 220, 221, 221, 222, 222, 222, 223,
    223, 224, 224, 224, 225, 225, 226, 226, 226, 227, 227, 228, 228, 228, 229,
    229, 230, 230, 230, 231, 231, 231, 232, 232, 233, 233, 233, 234, 234, 234,
    ...
    250, 250, 251, 251, 251, 251, 252, 252, 252, 253, 253, 253, 254, 254, 254,
    255};
static const uint32_t pow_035_table[256] = {
    0,   36,  46,  53,  59,  64,  68,  72,  75,  79,  82,  84,  87,  89,  92,
    94,  96,  98,  100, 102, 104, 106, 108, 109, 111, 113, 114, 116, 117, 119,
    120, 121, 123, 124, 125, 127, 128, 129, 130, 132, 133, 134, 135, 136, 137,
    138, 140, 141, 142, 143, 144, 145, 146, 147, 148, 149, 150, 150, 151, 152,
    153, 154, 155, 156, 157, 158, 158, 159, 160, 161, 162, 163, 163, 164, 165,
    166, 166, 167, 168, 169, 169, 170, 171, 172, 172, 173, 174, 175, 175, 176,
    177, 177, 178, 179, 179, 180, 181, 181, 182, 183, 183, 184, 185, 185, 186,
    186, 187, 188, 188, 189, 189, 190, 191, 191, 192, 192, 193, 194, 194, 195,
    195, 196, 197, 197, 198, 198, 199, 199, 200, 200, 201, 201, 202, 203, 203,
    204, 204, 205, 205, 206, 206, 207, 207, 208, 208, 209, 209, 210, 210, 211,
    211, 212, 212, 213, 213, 214, 214, 215, 215, 216, 216, 217, 217, 218, 218,
    218, 219, 219, 220, 220, 221, 221, 222, 222, 223, 223, 223, 224, 224, 225,
    225, 226, 226, 227, 227, 227, 228, 228, 229, 229, 230, 230, 230, 231, 231,
    232, 232, 232, 233, 233, 234, 234, 235, 235, 235, 236, 236, 237, 237, 237,
    238, 238, 239, 239, 239, 240, 240, 240, 241, 241, 242, 242, 242, 243, 243,
    244, 244, 244, 245, 245, 245, 246, 246, 247, 247, 247, 248, 248, 248, 249,
    249, 250, 250, 250, 251, 251, 251, 252, 252, 252, 253, 253, 253, 254, 254,
    255};
static const uint32_t pow_040_table[256] = {
    0,   27,  36,  43,  48,  52,  56,  60,  63,  66,  69,  72,  75,  77,  79,
    82,  84,  86,  88,  90,  92,  93,  95,  97,  99,  100, 102, 103, 105, 106,
    108, 109, 111, 112, 113, 115, 116, 117, 119, 120, 121, 122, 123, 125, 126,
    127, 128, 129, 130, 131, 132, 133, 134, 136, 137, 138, 139, 140, 141, 141,
    142, 143, 144, 145, 146, 147, 148, 149, 150, 151, 152, 152, 153, 154, 155,
    156, 157, 157, 158, 159, 160, 161, 161, 162, 163, 164, 165, 165, 166, 167,
    168, 168, 169, 170, 171, 171, 172, 173, 173, 174, 175, 176, 176, 177, 178,
    178, 179, 180, 180, 181, 182, 182, 183, 184, 184, 185, 186, 186, 187, 187,
    188, 189, 189, 190, 191, 191, 192, 192, 193, 194, 194, 195, 195, 196, 197,
    197, 198, 198, 199, 200, 200, 201, 201, 202, 202, 203, 204, 204, 205, 205,
    206, 206, 207, 207, 208, 208, 209, 210, 210, 211, 211, 212, 212, 213, 213,
    214, 214, 215, 215, 216, 216, 217, 217, 218, 218, 219, 219, 220, 220, 221,
    221, 222, 222, 223, 223, 224, 224, 225, 225, 226, 226, 227, 227, 228, 228,
    229, 229, 229, 230, 230, 231, 231, 232, 232, 233, 233, 234, 234, 235, 235,
    235, 236, 236, 237, 237, 238, 238, 239, 239, 239, 240, 240, 241, 241, 242,
    242, 242, 243, 243, 244, 244, 245, 245, 245, 246, 246, 247, 247, 248, 248,
...
static void draw_rect(uint8_t *target, uint8_t *base, uint32_t *diff, int width,
                      int x0, int y0, int w, int h, float colorR, float colorG,
                      float colorB, float colorA, uint8_t *cache) {
  uint8_t *t = target;
  uint8_t *c = nullptr;
  for (int y = h - 1; y >= 0; y--) {
...
static void load_textures() {
  if (loaded_textures)
    return;
  body_texture =
      cv::imread(Robot::get_texture_path("thymio-body-texture.png").string());
  led_texture_images[TOP_TEXTURE] = cv::imread(
      Robot::get_texture_path("thymio-body-diffusionMap0.png").string(),
      cv::IMREAD_UNCHANGED);
  led_texture_images[BOTTOM_TEXTURE] = cv::imread(
      Robot::get_texture_path("thymio-body-diffusionMap1.png").string(),
      cv::IMREAD_UNCHANGED);
  led_texture_images[LED_TEXTURE] = cv::imread(
      Robot::get_texture_path("thymio-body-diffusionMap2.png").string(),
      cv::IMREAD_UNCHANGED);
...
static std::array<std::string, 7> proximity_names = {
    "Left",  "CenterLeft", "Center",   "CenterRight",
    "Right", "RearLeft",   "RearRight"};
...
Thymio2::Thymio2(int handle_, uint8_t default_behavior_mask_)
    : Robot(handle_), default_behavior_mask(default_behavior_mask_),
      ...
      battery_voltage(3.61), temperature(22.0), mic_intesity(0.0),
      mic_threshold(0.0), r5(false), sd_card() {
  ...
  log_info("Initializing Thymio2 with handle %d and texture_id %d", handle,
           texture_id);
  ...
  for (size_t i = 0; i < proximity_names.size(); i++) {
    std::string prox_path =
        std::string(alias) + "/Proximity" + proximity_names[i];
    ...
    prox_comm.sensor_handles[i] =
        simGetObject((prox_path + "/Comm").c_str(), -1, -1, 0);
  }
...
Thymio2::~Thymio2() { reset_texture(false); }
...
void Thymio2::reset_texture(bool reload) {
  ...
  if (reload) {
    ...
    // printf("r %d, (%d, %d) %d \n", r, info.textureRes[0], info.textureRes[1],
    // info.indicesSize);
...
#endif
    texture_id =
        simApplyTexture(body_handle, textureCoords, info.indicesSize * 2,
                        (const unsigned char *)m.ptr(), info.textureRes, 1);
    // bottom-left corner is not mapped on the shape (i.e., the pixel is not
    // visible)

    log_debug("Reset texture for handle %d -> texture_id %d", handle,
              texture_id);
    ...
    simWriteTexture(texture_id, 0, (const char *)m.ptr(), 0, 0, TEXTURE_SIZE,
                    TEXTURE_SIZE, 0);
  }
}
...
void Thymio2::set_led_color(size_t index, bool force, float r, float g,
                            float b) {
  ...
  LED &led = leds[index];
  if (!force && !led.color.set_rgb(r, g, b))
    return;
  // printf("push led color %zu (%.2f %.2f %.2f %.2f)\n",
  //        index, led.color.r, led.color.g, led.color.b, led.color.a);
  uint8_t *base_image;
  uint32_t *led_image =
      led_texture_images[led_texture.texture_index].ptr<uint32_t>();
  if (index < 3) {
  ...
  for (auto &a : led_texture.regions) {
    ...
    simWriteTexture(texture_id, 0, (const char *)roi, a.x,
                    TEXTURE_SIZE - a.y - a.h, a.w, a.h, 0);
    free(roi);
  }
...
void Thymio2::update_sensing(float dt) { Robot::update_sensing(dt); }
...
static float prox_comm_sensor_response(float distance, float cos_emitter_angle,
                                       float cos_receiver_angle,
                                       float x0 = 2.0e-3, float c = 0.0917,
                                       float k_e = 18, float k_r = 9) {
  return (pow(abs(cos_emitter_angle), k_e) * pow(abs(cos_receiver_angle), k_r) *
          (c - x0 * x0) / (distance - x0) / (distance - x0));
}

static float prox_comm_response(float intensity, float m = 4600.0,
...
// - enki: I sum over all sectors; here: I just use 1 ray, not sure if this is
// still calibrated

static bool check_emitter_receiver(int emitter_handle, int receiver_handle,
                                   int receiver_sensor, float &distance,
  ...
  if (emitter_angle > max_emitter_angle)
    return false;
  ...
  // max_receiver_angle);
  if (receiver_angle > max_receiver_angle)
    return false;
  simFloat q[4];
  ...
  int r = simCheckProximitySensorEx(receiver_sensor, sim_handle_all, 5,
                                    distance - 1e-3, 0.0, detectedPoint,
                                    &detectedObjectHandle, nullptr);
  // printf("Collision? %d: %d (%d) at %.2f (%.2f)\n",
  // receiver_sensor, r, detectedObjectHandle, detectedPoint[3], distance -
  // 1e-3);
  return r == 0;
}

void ProximityComm::update_sensing(const std::array<int, 7> &tx_handles,
                                   int value) {
  ...
  for (size_t i = 0; i < 7; i++) {
    ...
    for (const int emitter : tx_handles) {
      ...
      // printf("emitter %d\n", emitter);
      if (check_emitter_receiver(emitter, receiver, sensor, distance, cos_e,
                                 cos_r)) {
